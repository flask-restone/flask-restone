...
from flask import current_app, json, jsonify, make_response, request, g
from flask.signals import Namespace
from flask_principal import UserNeed, RoleNeed, ItemNeed, Permission
...
# __all__ = ['Api','ModelResource','SQLAlchemyManager','principals']
#
# ---------------------------HTTP常量--------------------
    ...
    def as_dict(self):
        ...
        if self.id is not None:
            ...
            dct["item"] = {
                ...
                "$where": {cond.attribute: {
                    f"${cond.filter.name}": cond.value} if cond.filter.name is not None else cond.value
...
class FieldSet(Schema, ResourceBound):  # 字段集 规则和资源绑定
    ...
    def _on_bind(self, resource):  # 字段字典内部字段能绑则绑
        self.fields = {key: field.bind(resource) if isinstance(field,
                                                               ResourceBound) else field
    ...
    def _schema(self, patchable=False):  # _schema 内部规则
        read_schema = {
            ...
            "properties": OrderedDict(
                ((key, field.response) for (key, field) in self.fields.items()
                 if "r" in field.io)),
        }  # 响应的可读属性
        create_schema = {
            "type"                : "object",
            ...
            "properties"          : OrderedDict(
                ((key, field.request) for (key, field) in self.fields.items() if
                 "c" in field.io)),
        }  # 请求的可写属性
        update_schema = {
            "type"                : "object",
        ...
        for key, field in self.fields.items():
            if "c" in field.io and (not field.nullable) and (
    ...
    def readable_fields(self):  # 可读字段
        return {key: field for (key, field) in self.fields.items() if
                "r" in field.io}
    ...
    def all_fields_optional(self):  # 可选的字段
        return all((i.default is not None or i.nullable for i in
                    (self.fields or {}).values()))
    ...
    def convert(
        ...
        result = dict(
            pre_resolved_properties) if pre_resolved_properties else {}
        if patchable:
            object_ = self.patchable.convert(instance, update)  # 就是基类的转化校验
        else:  # 区别在于语法
            object_ = super().convert(instance, update)
        for key, field in self.fields.items():
            if update and "u" not in field.io or (
                    not update and "c" not in field.io):
                continue  # 不可更新字段或不更新
...
class Raw(Schema):
    ...
    def _finalize_schema(self, schema, io):  # 单个字典
        ...
        if "null" in schema.get("type", []):  # type 就是类型
            ...
            if "anyOf" in schema:  #
                if not any(("null" in choice.get("type", []) for choice in
                            schema["anyOf"])):
                ...
                if len(schema) == 1 and "$ref" in schema:  # 只有一个ref
                    ...
                    current_app.logger.warn(
                        f'{self} is nullable but "null" type cannot be added')
...
class Any(Raw):  # 可以用字典初始化
    def __init__(self, **kwargs):
        super().__init__({"type": ["null", "string", "number", "boolean",
                                   "object", "array"]}, **kwargs)
...
class Array(Raw, ResourceBound):
    def __init__(self, schema_cls_or_obj, min_items=None, max_items=None,
                 unique=None, **kwargs):
...
class Object(Raw, ResourceBound):
    def __init__(self, properties=None, pattern=None, pattern_props=None,
                 other_props=None, **kwargs):
        ...
        def schema():
            ...
            (request_schema, "request"), (response_schema, "response")):
                if self.properties:
                    schema["properties"] = {k: getattr(f, attr) for (k, f) in
                                            self.properties.items()}
                if self.pattern_props:
                    schema["patternProperties"] = {p: getattr(f, attr) for
                                                   (p, f) in
                                                   self.pattern_props.items()}
                if self.other_props:
                    schema["additionalProperties"] = getattr(self.other_props,
                                                             attr)
                else:
                    schema["additionalProperties"] = False
            return response_schema, request_schema
        
        if self.pattern_props and (
                len(self.pattern_props) > 1 or self.other_props):
            raise NotImplementedError(
    ...
    def bind(self, resource):
        if self.properties:
            self.properties = {key: _bind_schema(value, resource) for
                               (key, value) in self.properties.items()}
        if self.pattern_props:
            self.pattern_props = {key: _bind_schema(value, resource) for
                                  (key, value) in self.pattern_props.items()}
        if self.other_props:
    ...
    def formatter(self, value):
        output = {}
        if self.properties:
            output = {k: f.format(get_value(f.attribute or k, value, f.default))
                      for (k, f) in self.properties.items()}
        if self.pattern_props:  # 只能二选一，初始化的时候就抛出异常了，下面的异常不可达，删掉省一个判断
            field = next(iter(self.pattern_props.values()))
            output.update({k: field.format(v) for (k, v) in value.items() if
                           k not in self._property_attributes})
    ...
    def converter(self, instance):
        ...
        if self.properties:
            result = {field.attribute or key: field.convert(
                instance.get(key, field.default), ) for (key, field) in
                self.properties.items()}
        if self.pattern_props:
            field = next(iter(self.pattern_props.values()))
            result.update({key: field.convert(value, ) for (key, value) in
                           instance.items() if key not in result})
        elif self.other_props:
            field = self.other_props
            result.update(
                {key: field.convert(value) for (key, value) in instance.items()
...
class AttributeMapped(Object):
    ...
    def formatter(self, value):
        if self.pattern_props:
            field = next(iter(self.pattern_props.values()))
            return {get_value(self.mapping_attribute, v, None): field.format(v)
                    for v in value}
        if self.other_props:
            return {get_value(self.mapping_attribute, v,
                              None): self.other_props.format(v) for v in value}
        return {}
    
    def converter(self, value):
        if self.pattern_props:
            field = next(iter(self.pattern_props.values()))
            return [self._set_mapping_attribute(field.convert(v, ), k) for
                    (k, v) in value.items()]
        if self.other_props:
            return [self._set_mapping_attribute(self.other_props.convert(v), k)
...
class String(Raw):
    ...
    def __init__(self, min_length=None, max_length=None, pattern=None,
                 enum=None, format=None,
                 **kwargs):  # 参数用于类型检查 pattern 应为正则表达式 enum 应为枚举
        schema = {"type": "string"}
        if enum is not None:
            enum = list(enum)
        for v, k in (
...
class UUID(String):
    ...
    def __init__(self, **kwargs):
        super().__init__(min_length=36, max_length=36, pattern=self.UUID_REGEX,
                         **kwargs)
    ...
    def __init__(self, resource, **kwargs):  # resource可以是名称
        ...
        def schema():
            ...
            if len(key_converters) == 1:
                ...
                request_schema = {
                    "anyOf": [kc.request for kc in key_converters]}
            return response_schema, request_schema
        ...
        def schema():
            def _response_schema():
                if self.resource == self.target:
                    return {"$ref": "#"}
                return {"$ref": self.target.routes["describedBy"].rule_factory(
                    self.target)}
...
class BaseFilter(Schema):
    ...
    @property
    def filter_field(self):  # 被过滤的字段
        return self.field
    
    def _schema(self):  # 过滤器只能针对请求模式，过滤器的模式就是所过滤字段的请求模式
        return self.filter_field.request
    
    def _convert(self, value):
        return self.filter_field.convert(value, )  # 过滤器的转换就是所过滤字段的转换
    
    def convert(self, instance):
        if self.name is None:
            return Condition(self.attribute, self, self._convert(instance))
        return Condition(self.attribute, self,
                         self._convert(instance[f"${self.name}"]))
...
def simplify_schema_for_filter(schema):
    if schema:
        return {key: value for (key, value) in schema.items() if
                key not in ("readOnly",)}
    return schema
...
def filters_for_fields(
    ...
    for field_name, field in fields.items():
        field_filters = {
            name: filter for filter in
            filters_for_field_class(field.__class__, filters_by_type) for name
            in _get_names_for_filter(filter, filter_names)
        }
...
class PaginationMixin:  # 分页插件不能单独使用
    ...
    def format_response(self, data):
        ...
        headers = {
            "Link"         : ",".join(
                ['<{0}?page={1}&per_page={2}>; rel="{3}"'.format(*link) for link
                 in links]),
...
class Instances(PaginationMixin, Schema, ResourceBound):
    ...
    def _sort_fields(self):
        return {
            ...
            if
            name in self._filters and self.resource.manager._is_sortable_field(
                field)
        }
    ...
    def _filter_schema(self):
        return {
            "type"                : "object",
            "properties"          : {name: self._field_filters_schema(filters)
                                     for (name, filters) in
                                     self._filters.items()},
            "additionalProperties": False,
        }
    ...
    def parse_request(self, request):  # where 和 sort 是json字符串
        try:
            ...
            per_page = request.args.get("per_page", current_app.config[
                "RESTONE_DEFAULT_PER_PAGE"], type=int)
            style = current_app.config[
                "RESTONE_DEFAULT_PARSE_STYLE"]  # 新增了可配置的查询风格
            if style == "json":
...
class Pagination:
    ...
    @property
    def pages(self):
        return max(1, self.total // self.per_page + bool(
    ...
    def from_list(cls, items, page, per_page):
        ...
        return Pagination(items[start: start + per_page], page, per_page,
                          len(items))
...
class Manager:
    ...
    def _init_filter(self, filter_class, name, field, attribute):
        return filter_class(name, field=field,
    ...
    def _init_filters(self, resource, meta):
        ...
        self.filters = {
            field_name: {
                name: self._init_filter(filter, name, fields[field_name],
                                        field_name) for (name, filter) in
                field_filters.items()}
    ...
    def _init_key_converters(self, resource, meta):
        ...
        if "key_converters" in meta:
            meta.key_converters = [k.bind(resource) for k in
                                   meta["key_converters"]]
            meta.key_converters_by_type = {}
            for kc in meta.key_converters:
                if kc.matcher_type() in meta.key_converters_by_type:
                    raise RuntimeError(
                        f"Multiple keys of type {kc.matcher_type()} defined for {meta.name}")
    ...
    def _post_init(self, resource, meta):
        ...
        if meta.id_converter is None:
            meta.id_converter = getattr(meta.id_field_class,
                                        "url_rule_converter", None)
    ...
    def _get_field_from_python_type(python_type):
        try:
            ...
            raise RuntimeError(
                f'No appropriate field class for "{python_type}" type found')
...
class RelationalManager(Manager):
    ...
    def instances(self, where=None, sort=None):
        ...
        if query is None:
            return []
        if where:
            expressions = [self._expression_for_condition(condition) for
                           condition in where]
...
class PropertyKey(Key):
    ...
    def format(self, item):
        return self.resource.schema.fields[self.property].output(self.property,
                                                                 item)
...
class PropertiesKey(Key):
    ...
    def schema(self):
        return {
            "type"           : "array",
            "items"          : [self.resource.schema.fields[p].request for p in
                                self.properties],
            "additionalItems": False,
        }
    
    def format(self, item):
        return [self.resource.schema.fields[p].output(p, item) for p in
                self.properties]
    ...
    def convert(self, value):
        return self.resource.manager.first(
            where=[Condition(property, self._field_filters[property][None],
                             value[i]) for (i, property) in
...
class Relation(RouteSet, ResourceBound):  # 关系型也是RouteSet子类
    ...
    def routes(self):
        io = self.io
        rule = f"/{attribute_to_route_uri(self.attribute)}"  # /author
        relation_route = ItemRoute(
            rule=f"{rule}/<{self.target.meta.id_converter}:target_id>")  # /book/001/author/<sid>
        ...
        if "w" in io or "u" in io:
            ...
            def relation_add(resource, item, target_item):
                ...
                resource.manager.relation_add(item, self.attribute, self.target,
                                              target_item)
            ...
            yield relations_route.for_method(
                "POST",
                relation_add,
                rel=to_camel_case(f"add_{self.attribute}"),
            ...
            def relation_remove(resource, item, target_id):
                ...
                resource.manager.relation_remove(item, self.attribute,
            ...
            yield relation_route.for_method(
                "DELETE",
                relation_remove,
                rel=to_camel_case(f"remove_{self.attribute}"),
            )
...
class Route:
    def __init__(
        ...
        if isinstance(annotations, dict) and len(annotations):
            self.request_schema = FieldSet(
                {name: field for (name, field) in annotations.items() if
                 name != "return"})  # 请求的语法就是参数名和参数字段类型的字段集，响应也有字段
        ...
        for method in HTTP_METHODS:
            setattr(self, method,
                    MethodType(_method_decorator(method), self))  # 把方法绑定到类的实例中
            setattr(self, method.lower(),
                    getattr(self, method))  # 忽略大小写GET成为装饰器
    ...
    def relation(self):  # 关系型数据资源
        ...
        return to_camel_case(f"{verb}_{self.attribute}")
    ...
    def for_method(self, method, view_func, rel=None, title=None,
                   description=None, schema=None, response_schema=None,
                   **kwargs):
    ...
    def __get__(self, obj, owner):  # 返回的是个视图函数或类，obj是自己的对象，owner是自己的所属类
        if obj is None:
            return self
        return lambda *args, **kwargs: self.view_func.__call__(obj, *args,
                                                               **kwargs)
    ...
    def rule_factory(self, resource, relative=False):  # 规则工厂
        ...
        if rule is None:
            rule = f"/{attribute_to_route_uri(self.attribute)}"
...
def _field_from_object(parent, schema_cls_or_obj):  # 从对象获取字段
    ...
    if not isinstance(container, Schema):  # 实例不是格式类
        raise RuntimeError(
            f"{parent} expected Raw or Schema, but got {container.__class__.__name__}")
    ...
    def routes(self):
        ...
        if "r" in io:  # 读属性的路由
            ...
            def read_attribute(resource, item):
                if hasattr(resource,
                           f"before_read_{attribute}"):  # 直接调用source的钩子
                    getattr(resource, f"before_read_{attribute}")(
                        item)  # 也可以直接改为信号发射
                
                resp = get_value(attribute, item, field.default)
                
                if hasattr(resource,
                           f"after_read_{attribute}"):  # 直接调用source的钩子
            ...
            yield route.for_method(
                ...
                rel=to_camel_case(f"read_{route.attribute}"),
                # readDescription
            )
        if "u" in io:  # 更新属性的路由
            ...
            def update_attribute(resource, item, value):  # 直接post一个string即可
                if hasattr(resource,
                           f"before_update_{attribute}"):  # 直接调用source的钩子
                    getattr(resource, f"before_update_{attribute}")(item,
                                                                    value)  # 也可以直接改为信号发射
                
                item = resource.manager.update(item, {attribute: value})
                
                if hasattr(resource,
                           f"after_update_{attribute}"):  # 直接调用source的钩子
                    getattr(resource, f"after_update_{attribute}")(item,
                                                                   value)  # 也可以直接改为信号发射
            ...
            yield route.for_method(
                ...
                schema=field,
                response_schema=field,
                rel=to_camel_case(f"update_{route.attribute}"),
            )
...
class ResourceReference:
    ...
    def resolve(self, binding=None):
        ...
        if binding and binding.api:
            raise RuntimeError(
                f'Resource named "{name}" is not registered with the Api it is bound to.')
        raise RuntimeError(
            f'Resource named "{name}" cannot be found; the reference is not bound to an Api.')
...
class Resource(metaclass=ResourceMeta):
    ...
    def described_by(self):
        ...
        for prop in ("title", "description"):
            ...
            if value:
                schema[prop] = value
        links = [route for (name, route) in
                 sorted(self.routes.items())]  # name,Route
        if self.schema:
            schema["type"] = "object"
            schema.update(self.schema.response)
        schema["links"] = [link.schema_factory(self) for link in
                           sorted(links, key=operator.attrgetter("relation"))]
...
class ModelResourceMeta(ResourceMeta):
    def __new__(mcs, name, bases, members):
        class_ = super(ModelResourceMeta, mcs).__new__(mcs, name, bases,
                                                       members)
        meta = class_.meta
        if "Meta" in members:
            changes = members["Meta"].__dict__
...
def schema_to_swag_dict(schema, tags=None):
    ...
    if rel in ("self", "destroy") or rel.startswith("read"):
        parameters = [
            {"name"       : "id", "type": "string", "in": "path",
             "required"   : True, "description": "the data source's uuid"}]
        ...
        parameters = [
            {"name": "name", "type": "string", "in": "query", "default": ""},
    ...
    dct = {"tags"     : tags, "parameters": parameters, "responses": {
        "200": {"description": "正常返回", "examples": {"result": "success"}}}}
    return dct
...
class Api:
    ...
    def _deferred_blueprint_init(self, setup_state):
        ...
        for resource in self.resources.values():
            resource.route_prefix = "".join(
                (self.prefix, "/", resource.meta.name))
        self._init_app(setup_state.app)
    
    def _init_app(self, app):
        app.config.setdefault("RESTONE_MAX_PER_PAGE", 100)
        app.config.setdefault("RESTONE_DEFAULT_PER_PAGE", 20)
        app.config.setdefault("RESTONE_DEFAULT_PARSE_STYLE", "args")
    ...
    def _register_swag_view(self, app, route, resource, view_func):
        ...
        with app.app_context():
            ...
            tags = [
                resource.meta.title or resource.meta.name,
            ]
            print(schema)
    ...
    def _register_view(self, app, rule, view_func, endpoint, methods, relation):
        decorate_view_func = relation != "describedBy" or app.config[
            "RESTONE_DECORATE_SCHEMA_ENDPOINTS"]
        if self.blueprint:
        ...
        app.add_url_rule(rule, view_func=view_func, endpoint=endpoint,
                         methods=methods)
    ...
    def _exception_handler(self, original_handler, e):
        ...
        if isinstance(e, HTTPException):
            return _make_response({"status": e.code, "message": e.description},
                                  e.code)
        return original_handler(e)
    ...
    def _schema_view(self):
        ...
        for name, resource in sorted(self.resources.items(),
            ...
            resource_schema_rule = resource.routes["describedBy"].rule_factory(
                resource)
            properties[name] = {"$ref": "{}#".format(resource_schema_rule)}
        return OrderedDict(schema), 200, {
    ...
    def add_resource(self, resource):
        ...
        for route in resource.routes.values():
            route_decorator = resource.meta.route_decorators.get(route.relation,
                                                                 None)
        ...
        for name, rset in inspect.getmembers(resource,
                                             lambda m: isinstance(m, RouteSet)):
...
# 使用的时候 user = InlineModel({name},User)
...
class SQLAlchemyBaseFilter(BaseFilter):
    ...
    def apply(cls, query, conditions):
        expressions = [condition.filter.expression(condition.value) for
...
class SQLAlchemyManager(RelationalManager):
    ...
    def _init_model(self, resource, model, meta):
        ...
        self.id_field = self._get_field_from_column_type(self.id_column,
                                                         self.id_attribute,
                                                         io="r")
        self.default_sort_expression = self._get_sort_expression(model, meta,
                                                                 self.id_column)
        ...
        for name, column in mapper.columns.items():
            if (include_fields and name in include_fields) or (
                    exclude_fields and name not in exclude_fields) or not (
                ...
                fs.set(name,
                       self._get_field_from_column_type(column, name, io=io))
    ...
    def _get_field_from_column_type(self, column, attribute, io="rw"):
        ...
        if isinstance(column.type, postgresql.ARRAY):
            ...
            field_class = Object
            args = (String,)
        elif hasattr(postgresql, "JSON") and isinstance(column.type, (
        postgresql.JSON, postgresql.JSONB)):
            field_class = Raw
            kwargs = {"schema": {}}
        else:
            try:
    ...
    def update(self, item, changes, commit=True):
        ...
        actual_changes = {key: value for key, value in changes.items() if
                          self._is_change(get_value(key, item, None), value)}
    ...
    def relation_add(self, item, attribute, target_resource, target_item):
        before_add_to_relation.send(self.resource, item=item,
                                    attribute=attribute,
                                    child=target_item)  # 增加关联对象之前
        getattr(item, attribute).append(target_item)  # 一对多
        after_add_to_relation.send(self.resource, item=item,
                                   attribute=attribute, child=target_item)
    
    def relation_remove(self, item, attribute, target_resource, target_item):
        before_remove_from_relation.send(self.resource, item=item,
                                         attribute=attribute, child=target_item)
        try:
            getattr(item, attribute).remove(target_item)
            after_remove_from_relation.send(self.resource, item=item,
                                            attribute=attribute,
                                            child=target_item)
        except ValueError:
...
class HybridItemNeed(HybridNeed):  # HyHridItemNeed("creat","user") 创建用户的权限
    ...
    def __call__(self, item):
        ...
        return ItemNeed(self.method,
                        get_value(item, self.resource.manager.id_attribute,
                                  None), self.type)
    
    def __eq__(self, other):
        return isinstance(other,
                          HybridItemNeed) and self.method == other.method and self.type == other.type and self.resource == other.resource
...
class HybridRelationshipNeed(HybridItemNeed):
    def __init__(self, method, *fields):
        super().__init__(method, fields[-1].resource,
    ...
    def __eq__(self, other):
        return isinstance(other,
                          HybridItemNeed) and self.method == other.method and self.resource == other.resource and self.fields == other.fields
...
PERMISSION_DEFAULTS = (
("read", "yes"), ("create", "no"), ("update", "create"), ("delete", "update"))
...
PERMISSION_GRANTED_STRINGS = (
"yes", "everybody", "anybody", "everyone", "anyone")
...
class PrincipalMixin:  # 鉴权插件
                ...
                if need in PERMISSION_DENIED_STRINGS:  # 无权
                    ...
                    if isinstance(field, ToOne):  # 一对一的
                        ...
                        if role == "user":  # user:attr
                            options.add(
                                HybridUserNeed(field))  # 盲猜需要user的field字段为true
                        elif role == "role":  # role:xxx
                            options.add(RoleNeed(value))  # 需要用户的角色为xxx
                        else:  # 既不是user又不是role会是啥
                            for imported_need in target.manager._needs[
                                role]:  # 目标的_needmaps取
                    ...
                    elif role == "user" and value in ["$id",
                                                      "$uri"]:  # user:$id
                        options.add(HybridItemNeed("id",
                                                   self.resource))  # HybridItemNeed 需要id与当前用户id相同
                else:
    ...
    def get_permissions_for_item(self, item):  # 获取某个对象的权限
        # 这个权限可以用在这个方法操作当前item
        return {operation: permission.can(item) for operation, permission in
    ...
    def _query_filter_permission(self, query, permission):
        ...
        for need in permission.hybrid_needs:
            ...
            if len(need.fields) == 0:
                ...
                expression = need.fields[-1].target.manager._expression_for_ids(
                    ids)
    ...
    def relation_instances(self, item, attribute, target_resource, page=None,
        ...
        if page and per_page:
            return target_manager._query_get_paginated_items(query, page,
                                                             per_page)
