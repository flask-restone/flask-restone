...
class BaseField(Schema):
    ...
    def schema(self):
        ...
        if isinstance(schema, Schema):
            ...
            return self._finalize_schema(schema, "r"), self._finalize_schema(
                schema, "w")
        return self._finalize_schema(read_schema, "r"), self._finalize_schema(
            write_schema, "w")
...
class Any(BaseField):  # 可以用字典初始化
    def __init__(self, **kwargs):
        super().__init__(
            {"type": ["null", "string", "number", "boolean", "object",
                      "array"]},
            **kwargs,
        )
    ...
    def __init__(
        ...
        def schema():
            ...
            for _schema, attr in (
                ...
                if self.properties:
                    _schema["properties"] = {k: getattr(f, attr) for (k, f) in
                                             self.properties.items()}
                if self.pattern_props:  # 模式属性
                    _schema["patternProperties"] = {p: getattr(f, attr) for
                                                    (p, f) in
                                                    self.pattern_props.items()}  #
                if self.other_props:
                    _schema["additionalProperties"] = getattr(self.other_props,
                                                              attr)
                else:
...
class Inline(BaseField, ResourceMixin):  # 内联 默认不可更新 todo 设置可更新
    ...
    def convert(self, item, update=False, validate=True):  # 转换为输入 默认不可更新
        # if not validate:
        #     raise NotImplementedError()
        return self.target.schema.convert(item, update=update,
                                          patchable=self.patchable)
...
class BaseFilter(Schema):
    ...
    def field(self):  # 被过滤的字段,只是使用field.convert
        ...
        if not isinstance(self._field,
                          (Date, DateTime, DateString, DateTimeString)):
            return Number()
        return self._field
    ...
    def convert(self, instance, **kwargs):
        ...
        return Condition(self.attribute, self,
                         self.field.convert(instance[f"${self.name}"]))
...
ContainsFilter = filter_factory("has",
                                lambda a, b: hasattr(a, "__iter__") and b in a)
StringContainsFilter = filter_factory("ct", lambda a, b: a and b in a)
StringIContainsFilter = filter_factory("ict", lambda a,
                                                     b: a and b.lower() in a.lower())
StartsWithFilter = filter_factory("sw", lambda a, b: a.startswith(b))
IStartsWithFilter = filter_factory("isw",
                                   lambda a, b: a.lower().startswith(b.lower()))
EndsWithFilter = filter_factory("ew", lambda a, b: a.endswith(b))
IEndsWithFilter = filter_factory("iew",
                                 lambda a, b: a.lower().endswith(b.lower()))
...
class Instances(PaginationMixin, Schema, ResourceMixin):
    ...
    def __init__(self, required_fields=None,
                 item_decorator=None):  # 2.23 新增可选展示的字段
...
class Resource(metaclass=ResourceMeta):
    ...
    def described_by(self):
        ...
        schema["links"] = [link.schema_factory(self) for link in
                           sorted(links, key=attrgetter("relation"))]
    ...
    def create(self, properties):
        ...
        for k, v in properties.items():
            ...
            if isinstance(field, Inline):
                inst = field.target.manager.create(v,
                                                   commit=False)  # 使用目标字段的create方法，这样即使还有内联也可以直接创建
...
class RelationalManager(Manager):
    ...
    def instances(self, where=None, sort=None, options=None):
        ...
        if where:
            ...
            expressions = [
                self._expression_for_condition(condition) if isinstance(
                    condition, Condition) else condition for condition in where]
    ...
    @cached_property
    def _sort_fields(self):
        return {name: field for (name, field) in
                self.resource.schema.readable_fields.items() if
                name in self.filters and self._is_sortable_field(field)}
...
class PrincipalMixin:  # 鉴权插件
    ...
    def _query(self, **kwargs):
        ...
        if query is None and all(
                need.method == "role" for need in read_permission.needs):
            raise Forbidden()
